...
def main():
    ...
    if training_args.do_predict:
        ...
        hyps = predict_hyp(test_dataloader)
        results = evaluator.compute(hyps, refs, post_proc=True)
        ...
        if trainer.accelerator.is_main_process:
            ...
            with open(os.path.join(training_args.output_dir, "results.json"), "w") as f:
                json.dump(results, f)
            with open(
                os.path.join(training_args.output_dir, "hyps.txt"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write("\n".join(hyps))
