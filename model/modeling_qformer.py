...
class QFormerMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in QFormerModel forward() function)
...
class QFormerAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False):
        super().__init__()
        self.self = QFormerMultiHeadAttention(config, is_cross_attention)
...
class QFormerEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [QFormerLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class QFormerPreTrainedModel(PreTrainedModel):
    ...
    _keys_to_ignore_on_load_missing = [r"position_ids", "crossattention",]
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, QFormerEncoder):
...
class QFormerModel(QFormerPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(query_embeds)
